...
object CNF {
  ...
  implicit def toRichClause(c: Clause) = new {
    def toExpression = c.map(_.toExpression).reduceLeft(BOr)
  }
  implicit def toRichCNF(c: CNF) = new {
    def toExpression = c.map(_.toExpression).reduceLeft(BAnd)
  }
...
trait CNFRewriter extends Rewriter with Tseitin {
  ...
  val notValueRule = everywheretd {
    rule {
      case BNot(True) => False
      case BNot(False) => True
    }
  }
  ...
  val constantsRule = innermost {
    rule {
      case BOr(False,y) => y
      case BOr(x,False) => x
      case BAnd(False,_) | BAnd(_,False) => False

      case BAnd(True,y) => y
      case BAnd(x,True) => x
      case BOr(True,_) | BOr(_,True) => True

      case BImplies(True,y) => y
      case BImplies(False,_) => True
      case BImplies(_,True) => True
      case BImplies(x,False) => BNot(x)
    }
  }
